...
describe("Smart Search - extractKeywords & spanishStem", () => {
  ...
  it("should handle single word query", () => {
    ...
    const hasLaborLaw = results.some((r) => r.law.m?.some((m) => m.includes("Laboral")));
...
describe("Intelligent Search (Combined)", () => {
  ...
  it("should handle natural language well", () => {
    const results = intelligentSearch("quiero divorciarme de mi esposo", mockLaws, 10);
    expect(results.length).toBeGreaterThan(0);
  ...
  it("should handle complex queries", () => {
    const results = intelligentSearch("indemnizacion por despido arbitrario", mockLaws, 10);
    expect(results.length).toBeGreaterThan(0);
