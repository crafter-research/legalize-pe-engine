...
function calculateRelevance(
  ...
  const status = law.s ?? law.e;
  if (!status || status === "vigente" || status === "in_force") {
    score *= 1.2;
  }
 ...
 */
export function smartSearch(query: string, laws: CompactLey[], limit = 10): SearchResult[] {
 ...
 */
export function fuzzySearch(query: string, laws: CompactLey[], limit = 10): SearchResult[] {
 ...
 */
export function intelligentSearch(query: string, laws: CompactLey[], limit = 10): SearchResult[] {
